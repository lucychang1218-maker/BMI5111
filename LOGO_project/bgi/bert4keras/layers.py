...
# --- patch for TF legacy keras / tf_keras ---
keras_version = getattr(keras, "__version__", None)

need_nested_layer_patch = False
if keras_version is not None:
    try:
        if (not str(keras_version).endswith("tf")) and (version.parse(str(keras_version)) < version.parse("2.3")):
            need_nested_layer_patch = True
    except Exception:
        need_nested_layer_patch = False

if need_nested_layer_patch:
    class Layer(keras.layers.Layer):
        """重新定义Layer，赋予“层中层”功能（仅keras 2.3以下版本需要）"""
...
class Embedding(keras.layers.Embedding):
    ...
    def compute_mask(self, inputs, mask=None):
        ...
        """
        if getattr(self, "_current_mode", "embedding") == 'embedding':
