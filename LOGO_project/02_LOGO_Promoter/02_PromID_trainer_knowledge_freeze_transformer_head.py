...
- READ input NPZ:   epdnew_XXX_Knowledge_5_gram_frac5.npz / frac10.npz   (NO freeze tag in filename)
- WRITE outputs:    add FREEZE_TAG into task_name to separate ckpt/progress/metrics (YES freeze tag in outputs)
- Resume per fold: skip completed folds if ckpt exists
- Compute + eval metrics written into ONE metrics JSON per task per frac

✅ NEW:
- Freeze ALL layers except: (last Transformer block) + (head)
  i.e., only train Transformer-(num_hidden_layers-1) + BatchNorm/Dropout/Dense head
"""
...
def _progress_path(task_name: str, ngram: int, annotation_size: int, frac_tag: str) -> str:
    ...
    return os.path.join(
        METRICS_ROOT, "progress", frac_tag,
...
sys.path.insert(0, PROJECT_ROOT)  # so `import bgi...` works
...
# =========================
# ✅ Freeze policy: only train last Transformer block + head
# =========================
def freeze_all_but_last_transformer_and_head(model: tf.keras.Model, num_hidden_layers: int):
    """
    Keep trainable:
      - all layers whose name contains "Transformer-{last}-"
      - head layers we added: BatchNormalization, Dropout, CLS-Activation Dense, and the CLS Lambda
    Freeze everything else (embeddings + earlier transformer blocks + misc).
    """
    last_idx = num_hidden_layers - 1
    last_prefix = f"Transformer-{last_idx}-"

    def is_head_layer(layer: tf.keras.layers.Layer) -> bool:
        if layer.name == "CLS-Activation":
            return True
        if isinstance(layer, (tf.keras.layers.BatchNormalization, tf.keras.layers.Dropout, tf.keras.layers.Lambda)):
            return True
        return False

    for layer in model.layers:
        name = layer.name
        if (last_prefix in name) or is_head_layer(layer):
...
def train_kfold(
    ...
    # IMPORTANT: must match model_def_with_annotation_size num_hidden_layers
    NUM_HIDDEN_LAYERS = 2
    ...
    for fold, (train_idx, test_idx) in tqdm(
        ...
        with strategy.scope():
            ...
            # load pretrained first, then freeze, then compile
            ...
            freeze_all_but_last_transformer_and_head(model, num_hidden_layers=NUM_HIDDEN_LAYERS)
...
# =========================
# Main (READ raw npz; WRITE outputs with freeze tag)
# =========================
if __name__ == "__main__":
    # GPU memory growth (Colab-friendly)
    ...
    # ✅ output label only (ckpt/metrics/progress/task_name)
    FREEZE_TAG = "freeze_lastTransformer_head"

    # ✅ READ data base (NO freeze in filename)
    DATA_BASES = [
        {"data_base": "epdnew_BOTH_Knowledge",        "anno_size": 13, "batch": 128},
        {"data_base": "epdnew_NO_TATA_BOX_Knowledge", "anno_size": 13, "batch": 256},
        {"data_base": "epdnew_TATA_BOX_Knowledge",    "anno_size": 13, "batch": 128},
    ]

    data_path = f"./data/{ngram}_gram_11_knowledge"

    for frac_tag in FRAC_TAGS:
        for t in tqdm(DATA_BASES, desc=f"Tasks({frac_tag})", total=len(DATA_BASES), dynamic_ncols=True):
            data_base = t["data_base"]

            # ✅ READ: no freeze tag (must match your prepare script output)
            train_file = f"{data_base}_{ngram}_gram_{frac_tag}.npz"

            # ✅ WRITE: with freeze tag to distinguish experiments
            out_task_name = f"{data_base}_{FREEZE_TAG}"
