...
# 7) Main processing
# =========================================================
def process_and_save(sequence_dict, gene_features, df, subset_tag):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc=f"Encoding {subset_tag}"):
        ...
        e_hot = fetch_sequence(sequence_dict, raw_chrom_e, start_e, end_e, ENHANCER_LEN)
        p_hot = fetch_sequence(sequence_dict, raw_chrom_p, start_p, end_p, PROMOTER_LEN)
...
def main():
    ...
    if TASK == "train":
        ...
        process_and_save(sequence_dict, gene_features, df_frac10, "frac10_clean")
        process_and_save(sequence_dict, gene_features, df_frac20, "frac20_clean")
        process_and_save(sequence_dict, gene_features, df_frac50, "frac50_clean")

    elif TASK == "test":
        process_and_save(sequence_dict, gene_features, df_test, "test_clean")
